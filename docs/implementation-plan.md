...
1. **Missing buildings** (Phase 2) — the current 7 buildings are all Overture
   ...
   itself (D-31–D-33) — still not a full ground-truth inventory pass.
...
4. ~~**Building/overlay alignment** (Phase 4)~~ — done for the 5 buildings
   ...
   (D-29–D-33). House and Back Barn still use the ML-approximated positions,
...
- [x] Re-align building placement (and their roof-texture overlays) against the real drone orthomosaic/ground imagery — the 5 buildings under the front-section orthomosaic had position, size, and rotation re-derived directly from the orthomosaic pixels (D-29–D-33); House and Back Barn are unchanged, still the ML-approximated positions, pending the second drone flight
