...
- [x] Add fence lines and pasture divisions as simple planes/lines — fence now traces the real parcel boundary polygon (13 segments) as a procedurally-generated wire/post fence rather than a placeholder box (see D-11); the 5 numbered pastures east of Front Barn now also have interior dividing fences (9 more segments, same wire/post scene), derived from the real drone orthomosaic (D-61) — currently non-solid (`collision_layer = 0`, D-62) since the lines are still approximate. The rest of the property has no pasture-division lines yet. The property's two round pens (west of Front Barn, and between the House and Shop) are also now fenced, same post/wire look, on a `Path3D`/`Curve3D`-based scene (`scenes/fence_path.tscn`) rather than the straight `fence_segment.tscn` — currently placeholder circles at pixel-measured-from-orthomosaic coordinates (D-67/D-69) rather than hand-placed points; the user plans to trace the actual fence lines by clicking points directly against the visible ground texture in the editor in a future session (D-69)
...
- [ ] Model the east side of Sally's House as a real covered porch/entry addition (its own projecting volume — posts, depth, open sides) instead of a flat decal on the box — the user's own east-side photo shows a full porch structure there, not a plain gable wall (D-47). The *shape* of the gable cap above it is now measured and modeled (`gable_end_eave.gd`, 23.4° pitch with a fascia-overhang step, D-51), which is a real improvement over the old flat-triangle stopgap, but it's still a flat vertical plane, not a projecting volume — that needs a depth measurement or a side-angle photo this session doesn't have
- [ ] **KNOWN BROKEN (D-60):** the photo texture on `SallysHouse_Gable_East` and `SallysHouseGarage_Gable_North` — despite D-51 through D-59 (winding fix, roof overhang/extension, two rounds of UV-range bugs found and fixed, a tool feature to restore saved trace points), the user reports no visible change in-game. Every check this session could run (headless mesh/UV/material checks, eyeballing the exported crop images) passed, but an actual GPU render of the live scene was never achieved (blocked by host memory limits both times it was attempted), so something about how Godot's editor actually renders this isn't covered by anything verified so far. Next step: open the actual Godot editor (not headless) and look directly at these two nodes before changing anything else — texture import settings (filter/repeat/compression) and editor mesh/material caching are both unchecked and plausible
- [x] Give the garage a real sloped roof — north/south gable ends (with the same measured fascia-overhang shape as the house-east fix) and east/west sloped panels, photo-textured from the same front-section orthomosaic as the other buildings' roofs, pitch measured from the user's own photo (28.0°) rather than guessed (D-51)
...
- [x] Static horse models placed in pastures — CC0 rigged/animated horse model (Quaternius, see [decisions.md](project/decisions.md) D-9), now 14 named instances spread across the 5 numbered pastures east of Front Barn and a triangular dirt pasture west of it, each with its own roam rectangle derived from the real drone orthomosaic rather than one shared box (D-61)
- [x] Simple wander/movement AI — done ahead of the plan's stated ordering, at the user's request; picks a random point within its pasture's roam rectangle, walks to it, idles, repeats (`scripts/horse.gd`, generalized as `scripts/wander.gd` for the animals below). Horses now collide with the Player and each other (`CharacterBody3D` + `move_and_slide()`, D-63) and the walk animation loops properly on long walks (also D-63)
- [x] Rough per-horse coat coloring/patterning to match real photographed horses — flat color tints for solid-colored horses, triplanar-projected polka-dot textures standing in for appaloosa/pinto markings since the shared horse mesh has no UV coordinates (D-64)
...
- [x] One coyote (CC-BY, same source) wandering a fixed area in the back section of the property, away from the horse and flock — was found walking backwards (same forward-axis mismatch as the D-28 horse fix) and corrected the same way (D-65)
...
- [ ] Add any real animals not yet represented — current roster is 14 horses, a flock (6 hens + 1 rooster), 1 coyote, 3 ducks, 10 geese, 6 quail. Needs the real list of what's actually kept on the property today — current priority, see above
