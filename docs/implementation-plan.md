...
- [ ] Texture each building's exterior walls — only roofs have real photo textures so far (5 of 7 buildings, D-20 through D-23); the `Red Shipping Container` now has a real corrugated-metal texture on its top *and* sides, triplanar-tiled from a clean crop of its own top-down photo (D-39, no side-elevation photos exist for any building); every other building's walls are still the placeholder box color — current priority, see above
